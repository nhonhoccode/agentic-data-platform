...
def main() -> None:
    with httpx.Client(timeout=30.0) as client:
        capabilities = client.get(f"{BASE_URL}/api/v2/capabilities", headers=HEADERS)
        print(f"[{capabilities.status_code}] /api/v2/capabilities")
        print(json.dumps(capabilities.json(), indent=2, default=str)[:1200])
